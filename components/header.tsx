...
export default function Header()
{
    ...
    const {active,setTimeOfLastClick,setActive} = useActiveSectionContext();
            ...
            <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
                {
                    links.map((link) => (
                        <motion.li className ="h-3/4 flex items-center relative justify-center" key={link.hash}
                            ...
                            <Link className = {clsx("flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition",{ "text-gray-950":active===link.name,}
                            )}
                                  href={link.hash}
                                  onClick ={()=> {
                                      setActive(link.name);
